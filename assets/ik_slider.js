;(function ( $, window, document, undefined ) {
	...
	Plugin.prototype.init = function () {
		...
		if( !plugin.textfield.is(':text') ) {
			...
			plugin.textfield
				.attr({
					'readonly': '',
					'tabindex': -1	//remove ability to tab into this text field
				})
				.addClass('ik_value')
			...
			plugin.knob = $('<div/>')
				...
				.addClass('ik_knob')
				.on('keydown', {'plugin': plugin}, plugin.onKeyDown)	//add references for onKeyDown
	...
	Plugin.prototype.setValue = function (n) {
		...
		this.options.nowValue = n;
			//Dynamically set value of aria-valuenow
		this.knob
			.attr({
				'aria-valuenow':n
			});
